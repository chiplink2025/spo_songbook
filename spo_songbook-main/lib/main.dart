...
Future<List<Song>> readJSON() async {
  final contents = await File("songs.json").readAsString();
  final List<dynamic> jsonList = jsonDecode(contents);
  List<Song> songsList = [];

  for (final song in jsonList) {
    final String title = song.title;
    final String author = song.author;
    final String lyrics = song.lyrics;
    final int number = song.number;
    Song thisSong = Song(
        title: title,
        author: author,
...
class SongbookApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<Song>>(
      future: songsFuture, // Your Future<List<Song>>
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return MaterialApp(
              home: SongListScreen(songs: snapshot.data!)
          );
        } else {
